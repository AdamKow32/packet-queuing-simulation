...
namespace netsim {
    CbwfqScheduler::CbwfqScheduler(
        std::array<uint32_t, NUM_QOS_CLASSES> weights
    )
        : weights_(weights) {
        for (const uint32_t weight : weights_) {
            if (weight == 0U) {
                throw std::invalid_argument(
                    "CBWFQ weights must be positive"
                );
            }
        }
    }

    void CbwfqScheduler::enqueue(
        uint32_t packet_id,
        QoSClass qos_class,
        uint32_t size_bytes
    ) {
        ...
        queues_[cls_index].push(QueuedPacket{
            packet_id,
            finish_time
        });
    }
    ...
    uint32_t CbwfqScheduler::dequeue_next() {
        if (!has_packets()) {
            throw std::out_of_range(
                "CBWFQ scheduler queues are empty"
            );
        }
        ...
        for (std::size_t cls_index = 0; cls_index < queues_.size(); ++cls_index) {
            ...
            const double candidate_finish =
                queue.front().virtual_finish;

            if (candidate_finish < best_finish ||
                (
                    std::abs(candidate_finish - best_finish) < 1e-9 &&
                    cls_index < best_class
                )) {
        ...
        virtual_time_ =
            std::max(virtual_time_, packet.virtual_finish);
